...
st.set_page_config(page_title="Kesuksesan Akademik", layout="wide")

st.title("Kesuksesan Akademik: Lebih dari Sekadar Jam Belajar")
...
st.header("Overview Semua Nilai")
...
st.subheader("Korelasi antar Nilai G1, G2 dan G3")
...
# ============ BASIC DEMOGRAPHICS ============
st.header("Siapa Mereka? (Demografi Siswa)")
...
with col1:
    ...
    fig_gender = go.Figure(data=[
        go.Pie(labels=gender_counts.index, values=gender_counts.values, hole=0.5,
               marker=dict(colors=["#FF6B6B", "#45B7D1"]),
...
with col2:
    ...
    fig_famrel = px.bar(
        ...
        color=famrel_grade, color_continuous_scale='RdBu'
    )
...
with col2:
    if parent_analysis == "Pendidikan":
        ...
        fig_edu = px.box(
            ...
            color_discrete_map={'Ayah': '#45B7D1', 'Ibu': '#FF6B6B'},
        ...
        fig_job = px.bar(
            ...
            title=f'Pekerjaan Orang Tua vs {parent_grade}', color_discrete_map={'Ayah': '#45B7D1', 'Ibu': '#FF6B6B'}
        )
...
correlation_type = st.selectbox(
    ...
    ["Hubungan G1-G2-G3", "Faktor vs Semua Nilai"]
)
...
if correlation_type == "Hubungan G1-G2-G3":
    ...
    fig_pred = px.scatter(filtered_df, x='G1', y='G3', color='G2', 
                         ...
                         title="Nilai G3 berdasarkan G1 (warna = G2)",  color_continuous_scale='RdBu')
...
with comparison_col1:
    ...
    fig_gender_comp = px.histogram(
        ...
        title=f"Distribusi {gender_comparison_grade} berdasarkan Gender", color_discrete_map={'M': '#45B7D1', 'F': '#FF6B6B'}
    )
...
with comparison_col2:
    ...
    fig_school_comp = px.box(
        ...
        color='school', title=f"Distribusi {school_comparison_grade} per Sekolah", color_discrete_map={'GP': '#45B7D1', 'MS': '#FF6B6B'}
    )
...
### *Performa Akademik*
- Terdapat korelasi kuat antara **G1, G2, dan G3** — nilai awal memprediksi nilai akhir.
- Waktu belajar yang tinggi cenderung menghasilkan nilai lebih baik.
- Jumlah kegagalan sebelumnya berkorelasi negatif terhadap nilai akhir.

### *Faktor Dukungan*
- Dukungan dari keluarga dan sekolah berdampak positif pada performa.
- Siswa yang ingin melanjutkan pendidikan tinggi cenderung memiliki nilai lebih tinggi.
- Dukungan finansial (les berbayar) **tidak selalu** berkorelasi dengan performa.

### *Aspek Sosial & Gaya Hidup*
- Konsumsi alkohol berlebihan berkorelasi dengan penurunan nilai.
- Waktu luang tanpa struktur dapat menurunkan performa.
- Status hubungan romantis memiliki dampak yang bervariasi.

### *Latar Belakang Keluarga*
- Pendidikan orang tua, terutama **ibu**, berkorelasi positif dengan performa.
- Pekerjaan orang tua di bidang pendidikan atau kesehatan cenderung menghasilkan nilai akademik yang lebih baik.
- Kualitas hubungan keluarga (famrel) sangat berpengaruh terhadap stabilitas nilai.

---

## **Kesimpulan**

Kesuksesan akademik bukan hanya ditentukan oleh **seberapa banyak siswa belajar**, tetapi juga oleh **kesehatan lingkungan sosial, emosional, dan keluarga** di sekitar mereka.

Maka dari itu, penting bagi sekolah dan orang tua untuk menciptakan lingkungan yang mendukung secara **menyeluruh** — tidak hanya dari segi akademik, tetapi juga dari sisi **mental, sosial, dan motivasional** demi mendukung keberhasilan siswa secara berkelanjutan.
""")
